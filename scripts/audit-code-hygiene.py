...
EXPECTED_PAGE_TITLES = {
    ...
    "schedule.html": "Next @ the Caahhd Room | TLPT.org",
...
UNIFIED_TITLE_PAGES = {
    ...
    "schedule.html": "Next @ the Caahhd Room",
        ...
        if page.name == "voice-of-god.html":
            ...
            if not archive_path.is_file():
                ...
                try:
                    ...
                    clip_263 = clips_by_id.get("263", {})
                    expected_clip_263 = (
                        "Hiro caught bluffing again, and calmly admits it, which is "
                        "more honesty than the table deserves."
                    )
                    if (
                        str(clip_263.get("transcript", "")) != expected_clip_263
                        or clip_263.get("canonicalPlayersMentioned") != ["hiro"]
                        or clip_263.get("aliasesDetected") != ["Hiro"]
                    ):
                        parser.errors.append(
                    ...
                    clip_503 = clips_by_id.get("503", {})
                    expected_clip_503 = (
                        "BostnMike studied musical theater, and his poker range remains "
                        "mostly: The Sound of Mucking."
                    )
                    if str(clip_503.get("transcript", "")) != expected_clip_503:
                        parser.errors.append(
                            "Voice of God clip 503 must retain its approved transcript"
                        )
                    clip_526 = clips_by_id.get("526", {})
                    expected_clip_526 = (
                        "Ahmed says, ‘Yeah, man,’ a lot, usually immediately before "
                        "or after something requiring more serious analysis."
                    )
